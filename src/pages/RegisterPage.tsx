...
const RegisterPage = () => {
  ...
  const strength = useMemo(() => {
    ...
    if (passed <= 2) return { label: 'Fair', color: 'bg-[hsl(38,80%,50%)]', width: '50%' };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center w-full px-12 text-center">
          ...
          <p className="font-body text-muted-foreground font-light max-w-sm">
            Create stunning digital invitations that your guests will never forget.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-background">
        <div className="w-full max-w-md">
          ...
          <p className="font-body text-sm text-muted-foreground mb-8">Start building your invitation</p>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="font-body text-sm font-medium block mb-1.5">Full Name</label>
              <input {...register('name')} className="w-full rounded-xl border border-border bg-card px-4 py-3 font-body text-sm focus:outline-none focus:ring-2 focus:ring-gold/30" placeholder="Your full name" />
                ...
                <div className="mt-2">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                    {passwordRules.map(rule => (
                      <span key={rule.label} className={`font-body text-[11px] flex items-center gap-1 ${rule.test(password) ? 'text-emerald' : 'text-muted-foreground'}`}>
                        {rule.test(password) ? <Check size={11} /> : <X size={11} />} {rule.label}
                      </span>
                    ))}
                  </div>
                </div>
            ...
            <div>
              ...
              <input {...register('confirmPassword')} type="password" className="w-full rounded-xl border border-border bg-card px-4 py-3 font-body text-sm focus:outline-none focus:ring-2 focus:ring-gold/30" placeholder="••••••••" />
          ...
          <div className="grid grid-cols-3 gap-3">
            <button onClick={() => handleOAuth('Google')} className="flex items-center justify-center py-3 rounded-xl border border-border bg-card hover:bg-secondary transition-colors font-body text-sm">G</button>
            <button onClick={() => handleOAuth('Facebook')} className="flex items-center justify-center py-3 rounded-xl border border-border bg-card hover:bg-secondary transition-colors font-body text-sm">f</button>
            <button onClick={() => handleOAuth('Apple')} className="flex items-center justify-center py-3 rounded-xl border border-border bg-card hover:bg-secondary transition-colors font-body text-sm"></button>
          </div>

          <p className="text-center mt-8 font-body text-sm text-muted-foreground">
            Already have an account?{' '}
            <Link to="/login" className="text-gold font-medium hover:underline">Login →</Link>
          </p>
        </div>
      </div>
    </div>
