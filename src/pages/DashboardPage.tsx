...
const DashboardPage = () => {
  ...
  const totalViews = invitations?.reduce((sum, inv) => sum + (inv.viewCount || 0), 0) || 0;
  const totalRsvps = invitations?.reduce((sum, inv) => sum + (inv.rsvpCount || 0), 0) || 0;
  ...
  const shareWhatsApp = (inv: any) => {
    ...
    const msg = encodeURIComponent(`You're invited! Open your beautiful invitation here: ${url} 💌`);
            ...
            {invitations.map((inv, i) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => window.open(`/${inv.code}/invite/${inv.slug}`, '_blank')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-card font-body text-xs hover:bg-secondary transition-colors">
                    <Eye size={13} /> Preview
                  </button>
                  <button onClick={() => navigate(`/edit/${inv.id}`)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-card font-body text-xs hover:bg-secondary transition-colors">
                    <Pencil size={13} /> Edit
                  </button>
                  {inv.status === 'PUBLISHED' && (
                    <>
                      <button onClick={() => copyLink(inv.code || '', inv.slug)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-card font-body text-xs hover:bg-secondary transition-colors">
                        <Copy size={13} /> Copy Link
                      </button>
                      <button onClick={() => shareWhatsApp(inv)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[hsl(142,70%,40%)] text-white font-body text-xs hover:opacity-90 transition-opacity">
                        💚 WhatsApp
                      </button>
                    </>
                  )}
