        ...
        <thead className="bg-muted/50">
          <tr>
            {[
              ...
              <th
                key={h}
                className="px-4 py-3 text-left font-body font-medium text-muted-foreground"
              >
                {h}
              </th>
            ))}
          ...
          {quotes?.map((q: any) => (
            ...
            >
              ...
              <td className="px-4 py-3">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-body font-medium ${STATUS_COLORS[q.status] || STATUS_COLORS.CLOSED}`}
                >
                  {q.status}
                </span>
              </td>
              <td className="px-4 py-3">
                <select
                  value={q.status}
                  onChange={(e) =>
                    statusMut.mutate({ id: q.id, status: e.target.value })
                  }
