...
const EditablePhoto = ({
    ...
    return (
      <div
        ...
        className={cn(
          ...
          mode === 'edit' && 'cursor-pointer hover:border-primary/50 hover:bg-primary/5 transition-all'
        )}
      >
        <input
          ref={inputRef}
        ...
        {isUploading ? (
          ...
          <>
            ...
            {mode === 'edit' && (
              <span className="font-body text-xs text-muted-foreground">{placeholderText}</span>
            )}
          </>
        )}
      </div>
  ...
  return (
    <div
      ...
      className={cn(
        baseClasses,
        mode === 'edit' && 'cursor-pointer group'
      )}
    >
      <input
        ref={inputRef}
      ...
      {mode === 'edit' && (
        <div className={cn(
          ...
          {isUploading ? (
            <>
              <Loader2 className="w-8 h-8 text-primary animate-spin mb-2" />
              <span className="font-body text-sm text-foreground">{uploadProgress}%</span>
            </>
          ) : (
            <>
              <Camera className="w-8 h-8 text-foreground mb-2" />
              <span className="font-body text-sm text-foreground">Change Photo</span>
            </>
          )}
        </div>
      )}
