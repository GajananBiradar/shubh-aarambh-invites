...
import { ChevronDown, Heart, MapPin, Calendar, Clock, Copy, Eye } from "lucide-react";
...
const IvoryGardenTemplate = ({
  ...
  return (
    <div data-theme="ivory" className="min-h-screen bg-background text-foreground">
      {/* ═══════════════════════════════════════
          HERO — Dark with gold ornamental frame, NO photo bg
          ═══════════════════════════════════════ */}
      <section className="relative min-h-[100dvh] flex flex-col items-center justify-center overflow-hidden"
        style={{ background: 'linear-gradient(180deg, hsl(225 50% 8%) 0%, hsl(225 40% 12%) 100%)' }}
      >
        {/* Gold particle shimmer */}
        ...
        <div className="relative z-10 text-center px-4">
          ...
          {mode === "edit" ? (
            <div className="mt-4">
              <label className="font-body text-xs opacity-70 block mb-1 tracking-wider uppercase">Wedding Date</label>
              <input
                type="date"
        ...
        >
          <p className="font-body text-xs opacity-50 mb-2 tracking-wider uppercase">Scroll</p>
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-12">
            ...
            >
              ...
              <h3 className="font-heading text-2xl" style={{ color: 'hsl(45, 100%, 55%)' }}>
      ...
      {(data.showCountdown || mode === "edit") && (
        ...
        >
          <div className="container mx-auto px-4 max-w-2xl text-center">
            ...
            {data.showCountdown && (
              <>
                <p className="font-body text-xs tracking-[0.3em] uppercase mb-8" style={{ color: 'hsl(45, 100%, 55%)' }}>
                  Counting Down
                </p>
      ...
      >
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="text-center mb-16">
            <p className="font-body text-xs tracking-[0.3em] uppercase mb-4" style={{ color: 'hsl(45, 100%, 55%)' }}>
              The Celebrations
            </p>
                    ...
                    {i % 2 === 0 ? (
                      <>
                        <div className="text-right hidden md:block">
                          <p className="font-body text-sm opacity-60">{formatEventDate(event.eventDate)}</p>
                          <p className="font-body text-sm opacity-60">{formatTime(event.eventTime)}</p>
                        </div>
                        <div className="bg-[hsl(225,35%,14%)] rounded-xl p-6 border border-[hsl(45,100%,50%,0.15)]">
                          ...
                          <h3 className="font-heading text-xl mb-2" style={{ color: 'hsl(45, 100%, 55%)' }}>{event.eventName}</h3>
                          <p className="font-body text-sm">{event.venueName}</p>
                          <p className="font-body text-xs opacity-60">{event.venueAddress}</p>
                          {event.mapsUrl && (
      ...
      >
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <p className="font-body text-xs tracking-[0.3em] uppercase mb-4" style={{ color: 'hsl(45, 100%, 55%)' }}>Moments</p>
      ...
      {/* RSVP (view/demo) */}
      {mode !== "edit" && <GoldRsvpSection invitationId={data.invitationId} isDemo={mode === "demo"} />}

      {/* Footer */}
      <footer className={cn("py-20 text-center", mode === "edit" && "pb-32")}
        style={{ background: 'hsl(225, 45%, 8%)' }}>
        <div className="container mx-auto px-4">
          <div className="w-16 h-px mx-auto mb-6" style={{ background: 'hsl(45, 100%, 50%, 0.4)' }} />
          <p className="font-heading text-3xl" style={{ color: 'hsl(45, 100%, 55%)' }}>
          ...
          {data.hashtag && <p className="font-body text-sm mt-4" style={{ color: 'hsl(45, 100%, 55%)' }}>{data.hashtag}</p>}
          <p className="font-body text-[10px] opacity-30 mt-10">Made with love on ShubhAarambh</p>
        </div>
      </footer>
...
const GoldCountdownDisplay = ({ weddingDate }: { weddingDate: string }) => {
  ...
  if (!countdown) return <p className="font-body opacity-60">Set wedding date to see countdown</p>;
      ...
      {[
        ...
        <div key={i} className="text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 border border-[hsl(45,100%,50%,0.4)] rounded-lg flex items-center justify-center mb-2">
            <span className="font-heading text-3xl sm:text-4xl font-bold" style={{ color: 'hsl(45, 100%, 55%)' }}>
              {String(item.value).padStart(2, "0")}
            </span>
          </div>
          <span className="font-body text-xs opacity-50 tracking-wider uppercase">{item.label}</span>
        </div>
      ))}
    </div>
  );
...
/* Dark themed RSVP */
const GoldRsvpSection = ({ invitationId, isDemo }: { invitationId: number | null; isDemo: boolean }) => {
    ...
    return (
      <section className="py-20" style={{ background: 'hsl(225, 40%, 10%)' }}>
        <div className="container mx-auto px-4 max-w-md text-center">
          <Heart className="w-10 h-10 mx-auto mb-4" style={{ color: 'hsl(45, 100%, 55%)' }} />
          <h3 className="font-heading text-2xl mb-2">Thank You</h3>
          <p className="font-body text-sm opacity-60">Your response has been recorded.</p>
        </div>
      </section>
    );
  }
    ...
    <section className="py-20" style={{ background: 'hsl(225, 40%, 10%)' }}>
      <div className="container mx-auto px-4 max-w-md">
        <div className="text-center mb-10">
          <p className="font-body text-xs tracking-[0.3em] uppercase mb-4" style={{ color: 'hsl(45, 100%, 55%)' }}>Respond</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {attending && attending !== "no" && (
            <div className="space-y-4">
              ...
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number"
                className="w-full px-4 py-3 rounded-lg border border-[hsl(225,25%,20%)] bg-transparent font-body text-sm focus:outline-none focus:border-[hsl(45,100%,50%,0.5)]" />
              <div>
                <label className="font-body text-sm opacity-60 block mb-2">Number of Guests</label>
          ...
          <button type="submit" disabled={!attending}
            className={cn("w-full py-3 rounded-lg font-body font-medium text-sm tracking-wider uppercase transition-all",
              attending ? "border border-[hsl(45,100%,50%)] text-[hsl(45,100%,55%)] hover:bg-[hsl(45,100%,50%,0.1)]" : "opacity-30 cursor-not-allowed border border-[hsl(225,25%,20%)]"
            )}>
            Confirm Response
          </button>
        </form>
